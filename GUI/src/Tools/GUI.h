...
class GUI
{
    public:
         ...
         : showcaseMode(showcaseMode)
        {
            ...
            pangolin::CreateWindowAndBind("", width, height, windowParams);
            ...
            if(!showcaseMode)
            {
                pangolin::CreatePanel("ui").SetBounds(0.0, 0.5, 0.0, pangolin::Attach::Pix(panel));
            ...
            step = new pangolin::Var<bool>("Step", false, false);
            save = new pangolin::Var<bool>("ui.Save", false, false);
            reset = new pangolin::Var<bool>("ui.Reset", false, false);
            flipColors = new pangolin::Var<bool>("Flip RGB", false, true);

            if(liveCap)
            {
                autoSettings = new pangolin::Var<bool>("Auto Settings", true, true);
            }
            else
            {
                autoSettings = 0;
            ...
            pyramid = new pangolin::Var<bool>("Pyramid", true, true);
            so3 = new pangolin::Var<bool>("SO(3)", true, true);
            frameToFrameRGB = new pangolin::Var<bool>("Frame to frame RGB", false, true);
            fastOdom = new pangolin::Var<bool>("Fast Odometry", false, true);
            rgbOnly = new pangolin::Var<bool>("RGB only tracking", false, true);
            confidenceThreshold = new pangolin::Var<float>("Confidence threshold", 10.0, 0.0, 24.0);
            
            icpWeight = new pangolin::Var<float>("ICP weight", 10.0, 0.0, 100.0);

            followPose = new pangolin::Var<bool>("ui.Follow pose", true, true);
            drawRawCloud = new pangolin::Var<bool>("Draw raw", false, true);
            drawFilteredCloud = new pangolin::Var<bool>("Draw filtered", false, true);
            drawGlobalModel = new pangolin::Var<bool>("ui.Draw global model", true, true);
            drawUnstable = new pangolin::Var<bool>("Draw unstable points", false, true);
            drawPoints = new pangolin::Var<bool>("ui.Draw points", false, true);
            
	        drawColors = new pangolin::Var<bool>("ui.Draw colors", true, true);
            drawFxaa = new pangolin::Var<bool>("Draw FXAA", showcaseMode, true);
            drawWindow = new pangolin::Var<bool>("Draw time window", false, true);
            drawNormals = new pangolin::Var<bool>("Draw normals", false, true);
            drawTimes = new pangolin::Var<bool>("Draw times", false, true);
            drawDefGraph = new pangolin::Var<bool>("Draw deformation graph", false, true);
            drawFerns = new pangolin::Var<bool>("Draw ferns", false, true);
            drawDeforms = new pangolin::Var<bool>("Draw deformations", true, true);
            depthCutoff = new pangolin::Var<float>("ui.Depth cutoff", 0.4, 0.0, 1.0);

            //规则体拟合
            Cuboid = new pangolin::Var<bool>("ui.Cuboid", false, false);
            Cone = new pangolin::Var<bool>("ui.Cone", false, false);
            Cylinder = new pangolin::Var<bool>("ui.Cylinder", false, false);
            

            //===================================================================
            gpuMem = new pangolin::Var<int>("GPU memory free", 0);

            totalPoints = new pangolin::Var<std::string>("Total points", "0");
            totalNodes = new pangolin::Var<std::string>("Total nodes", "0");
            totalFerns = new pangolin::Var<std::string>("Total ferns", "0");
            totalDefs = new pangolin::Var<std::string>("Total deforms", "0");
            totalFernDefs = new pangolin::Var<std::string>("Total fern deforms", "0");

            trackInliers = new pangolin::Var<std::string>("Inliers", "0");
            trackRes = new pangolin::Var<std::string>("Residual", "0");
            logProgress = new pangolin::Var<std::string>("Log", "0");
